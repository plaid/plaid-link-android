...
import com.plaid.link.result.Link
import com.plaid.link.result.LinkSuccess
import com.plaid.link.network.LinkTokenRequester
...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    openButton = findViewById(.id.open_link)
  ...
  /**
   * Optional, set an [event listener](https://plaid.com/docs/link/android/#handlingevents).
   */
  ...
  private fun openLink() {
    ...
    plaidHandler?.let { linkAccountsToPlaid.launch(it) }
  }
  ...
  private fun onLinkToken( Throwable) {
    if (js java.net.ConnectException) {
      Toast.makeText(this, "Please run `sh start_server.sh <client_id> <sandbox_secret>`", 
      return
    }

  }

  override fun onCreateOptionsMenu(menu): Boolean {
  ...
  private fun showSuccess(success: LinkSuccess) {
    ...
    result.text = getString(R.string.content_success)
      ...
      result.text = getString(string.content_.metadata.status?.jsonValue ?: )
    }
  }
}
